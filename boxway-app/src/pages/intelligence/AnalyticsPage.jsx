...
const AnalyticsPage = () => {
  ...
  const getFilteredData = () => {
    const months = period === '3M' ? 3 : period === '7M' ? 7 : 12;
    const multiplier = period === '3M' ? 0.3 : period === '7M' ? 0.7 : 1;
    return {
      filteredRevenue: revenueByMonth.slice(-months),
      filteredKPIs: {
        ...kpis,
        totalRevenue: kpis.totalRevenue * multiplier,
        activeProjects: period === '3M' ? Math.floor(kpis.activeProjects * 0.5) : period === '7M' ? Math.floor(kpis.activeProjects * 0.8) : kpis.activeProjects,
        proposalWinRate: period === '3M' ? Math.floor(kpis.proposalWinRate * 0.8) : period === '7M' ? Math.floor(kpis.proposalWinRate * 0.9) : kpis.proposalWinRate,
        teamUtilization: period === '3M' ? '75%' : period === '7M' ? '82%' : '87%',
      },
      filteredProjectsByStatus: projectsByStatus.map(item => ({
        ...item,
        count: period === '3M' ? Math.floor(item.count * 0.4) : period === '7M' ? Math.floor(item.count * 0.7) : item.count
      }))
    };
  };

  const { filteredRevenue, filteredKPIs, filteredProjectsByStatus } = getFilteredData();
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8 bg-[#fcf9f8]">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-black text-slate-900">Analytics & Reports</h2>
          <p className="text-sm text-slate-500 font-medium">Business performance insights for your architecture firm</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleExport} className="flex items-center gap-2 px-4 py-2 border border-slate-200 bg-white text-slate-700 text-sm font-semibold rounded hover:bg-slate-50">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Revenue (YTD)', val: '$' + (filteredKPIs.totalRevenue / 1000).toFixed(0) + 'K', sub: `+${kpis.revenueGrowth}% vs last year`, icon: 'trending_up', color: 'text-green-600', bg: 'bg-green-50' },
          { label: 'Active Projects', val: filteredKPIs.activeProjects, sub: 'Currently in progress', icon: 'architecture', color: 'text-blue-600', bg: 'bg-blue-50' },
          { label: 'Proposal Win Rate', val: filteredKPIs.proposalWinRate + '%', sub: 'Past 6 months', icon: 'thumb_up', color: 'text-primary', bg: 'bg-primary/10' },
          ...
          { label: 'Team Utilization', val: filteredKPIs.teamUtilization, sub: 'Average across teams', icon: 'group', color: 'text-teal-600', bg: 'bg-teal-50' },
        ].map(k => (
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-8 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <div className="flex justify-between items-center mb-5">
            <h3 className="font-bold text-slate-900 text-sm uppercase tracking-wider">Revenue vs Expenses</h3>
          ...
          <BarChart data={filteredRevenue} keyX="month" keyA="revenue" keyB="expenses" labelA="Revenue" labelB="Expenses" />
        </div>

        {/* Project Status */}
        <div className="col-span-4 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h3 className="font-bold text-slate-900 text-sm uppercase tracking-wider mb-4">Projects by Status</h3>
          <div className="space-y-4">
            {filteredProjectsByStatus.map(item => {
              const max = filteredProjectsByStatus[0].count;
              return (
                <div key={item.status}>
        ...
        <div className="col-span-5 bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <h3 className="font-bold text-slate-900 text-sm uppercase tracking-wider mb-4">Top Clients by Revenue</h3>
          <div className="space-y-4">
            {topClients.map((c, i) => {
        ...
        <div className="col-span-7 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100">
            <h3 className="font-bold text-slate-900 text-sm uppercase tracking-wider">Monthly Performance</h3>
          </div>
